...
class audit {
	...
	public function listTypes($selected) {
		...
		$response="";
		while ($row=$result->fetch_assoc()) {
			$response.='<option value="' . $row["control_name"] . '"';
	...
	public function addAudit($company, $industry, $team, $task, $assessor, $site, $date, $time, $street, $building, $zipcode, $state, $country, $existing, $control, $audi_treatment) {
		$db = new db();
		$conn = $db->connect();
	...
	public function editAudit($id, $company, $industry, $team, $task, $assessor, $site, $date, $time, $street, $building, $zipcode, $state, $country, $control, $audi_treatment) {
		...
		$query="UPDATE as_auditcontrols SET " . 
				...
				" aud_treatment='" . $audi_treatment . "' WHERE idcontrol=" . $id;
